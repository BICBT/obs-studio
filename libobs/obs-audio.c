...
static void find_source_by_reference(obs_source_t *parent, obs_source_t *source, void *param) {
    UNUSED_PARAMETER(parent);
    struct find_source_param *p = param;
    p->find = p->find || source == p->find_source;
}

static bool find_source_in_root_nodes(obs_source_t *source, struct obs_core_audio *audio) {
    ...
    for (size_t i = 0; i < audio->root_nodes.num; i++) {
        ...
        obs_source_enum_active_tree(root_source, find_source_by_reference, &param);
        if (param.find) {
            return true;
        }
    }
